...
public:
	CPropertyColorItem(LPCTSTR pstrName, LPARAM lParam) :
		CPropertyItem(pstrName, lParam),
		m_hwndEdit(NULL)
	{
	}

	CPropertyColorItem(LPCTSTR pstrName, DWORD colorref, LPARAM lParam) :
		CPropertyItem(pstrName, lParam),
		m_hwndEdit(NULL)
	{
		m_val = colorref;
	...
	BOOL GetDisplayValue(LPTSTR pstr, UINT cchMax) const
	{
		...
		COLORREF color = m_val.uintVal;
		wsprintf(pstr, L"%d, %d, %d"
			, GetRValue(color)
			, GetGValue(color)
			, GetBValue(color));
	...
	UINT GetDisplayValueLength() const
	{
		...
		WCHAR buffer[64];
		GetDisplayValue(buffer, 63);

		return v.bstrVal == NULL ? 0 : ::lstrlen(buffer);
	}
	...
	void DrawValue(PROPERTYDRAWINFO& di)
	{
		...
		RECT rcColor = di.rcItem;
		rcColor.left += 2;
		rcColor.top += 2;
		rcColor.right = rcColor.left + 16;
		rcColor.bottom -= 2;
